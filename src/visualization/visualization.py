...
from src.visualization.echart_html import (
    html_accuracy_loss_chart,
    html_heatmap_chart,
    html_heatmap_chart_2,
)
...
class VisualizeTraining:
    ...
    def _plot_results(
        self, history, mode, title, ylabel="", xlabel="Epoch", historytags: list = []
    ) -> None:
    ...
    def plot_interactive_training_results(self, history):
        ...
        for x_axis_nr in range(0, len(history_data["accuracy"])):
            ...
            list_accuracy.append(
                round(history_data["accuracy"][x_axis_nr], rounded_accuracy)
            )
            list_val_accuracy.append(
                round(history_data["val_accuracy"][x_axis_nr], rounded_accuracy)
            )
            list_loss.append(round(history_data["loss"][x_axis_nr], rounded_loss))
            list_val_loss.append(
                round(history_data["val_loss"][x_axis_nr], rounded_loss)
            )
        ...
        filename = self._save_interactive_plot_html_2(
            "val_acc_and_loss", data_info_html
        )
    ...
    def plot_interactive_adversarial_training_results(self, history):
        ...
        for x_axis_nr in range(0, len(history_data["accuracy"])):
            list_x_value.append(x_axis_nr + 1)
            list_accuracy.append(
                round(history_data["accuracy"][x_axis_nr].item(), rounded_accuracy)
            )
            list_val_accuracy.append(
                round(history_data["val_accuracy"][x_axis_nr].item(), rounded_accuracy)
            )
            list_loss.append(
                round(history_data["loss"][x_axis_nr].item(), rounded_loss)
            )
            list_val_loss.append(
                round(history_data["val_loss"][x_axis_nr].item(), rounded_loss)
            )
        ...
        data_info_html = html_accuracy_loss_chart(
            data_info, "Adversarial Training Accuracy & Loss"
        )
        filename = self._save_interactive_plot_html_2(
            "adv_train_acc_loss", data_info_html
        )
...
class VisualizeEvaluation:
    ...
    def plot_predictions(
        ...
        classes: list,
        num_samples: int = 25,
        filename_text: str = "plot_file_names",
    ) -> None:
        ...
        for i in range(num_samples):
            ...
            if x_test.shape[-1] == 1:  # Grayscale images
                plt.imshow(
                    x_test[i].reshape(x_test.shape[1], x_test.shape[2]), cmap="gray"
                )
            else:  # RGB images
                plt.imshow(
                    x_test[i].reshape(x_test.shape[1], x_test.shape[2], x_test.shape[3])
                )
            plt.title(
                f"True: {classes[y_true[i]]}, Predicted: {classes[predicted_labels[i]]}"
            )
            plt.axis("off")
        plt.tight_layout()
    ...
    def plot_confusion_matrix(
        self, y_true: np.ndarray, y_pred: np.ndarray, classes: list
    ) -> None:
    ...
    def plot_interactive_confusion_matrix(self, y_true, y_pred, classes):
        cm = confusion_matrix(y_true, y_pred)
        data_info = [
            {"value": int(cm[x][y]), "row": classes[x], "column": classes[y]}
            for x in range(len(cm))
            for y in range(len(cm[x]))
        ]
        data_info_html = html_heatmap_chart(data_info, classes)
        filename = self._save_interactive_plot_html_2(
            "confusion_matrix", data_info_html
        )
    ...
    def plot_interactive_classification_report(
        self, y_true, y_pred_classes, classes, output_dict=True
    ):
        report = classification_report(
            y_true,
            y_pred_classes,
        ...
        filename = self._save_interactive_plot_html_2(
            "classification_report", data_info_html
        )
    ...
    def plot_adversarial_examples(
        ...
        for i in range(num_samples):
            ...
            plt.title(
                f"Clean\nPred:\n {classes[np.argmax(model.predict(x_test[i:i + 1]), axis=1)[0]]}",
                fontsize=10,
            )
            plt.axis("off")
    ...
    def plot_accuracy_comparison(
        self, accuracies: list, labels: list = ["Clean", "FGSM", "PGD"]
    ) -> None:
    ...
    def plot_interactive_accuracy_comparison(
        self, accuracies, labels=["Clean", "FGSM", "PGD"]
    ):
        ...
        fig = plotly_graph_objects.Figure(
            [
                plotly_graph_objects.Bar(
                    x=labels, y=accuracies, marker_color=["blue", "green", "red"]
                )
            ]
        )
        ...
        fig.update_layout(
            ...
            title_font_size=20,
            xaxis=dict(
                title="Model", tickmode="array", tickvals=bar_positions, ticktext=labels
            ),
    ...
    def plot_file_names(self) -> dict:
        ...
        if len(self._plot_file_names) < 1:
            raise ValueError(
                StringStyling.box_style(
                    message="visualizer: missing file names in dict"
                )
            )
        return self._plot_file_names
...
class VisualizeUncertainty:
    ...
    def plot_interactive_pcs_mean_softmax(self, pcs_mean_softmax_scores):
        ...
        fig = make_subplots(
            ...
            cols=1,
            subplot_titles=(
                "Distribution of PCS Scores",
                "Distribution of Mean Softmax Scores",
            ),
        )
        fig.add_trace(
            plotly_graph_objects.Histogram(
                x=pcs_scores, name="PCS Scores", marker_color="skyblue"
            ),
            row=1,
            col=1,
        )
        ...
        fig.update_layout(
            title_text="", xaxis_title_text="", yaxis_title_text="", bargap=0.2
        )
    ...
    def plot_interactive_distribution_pcs_ms_scores(self, pcs_mean_softmax_scores):
        ...
        fig.add_trace(
            plotly_graph_objects.Histogram(
                x=pcs_scores, name="PCS", marker_color="blue", nbinsx=100
            )
        )
        fig.add_trace(
            plotly_graph_objects.Histogram(
    ...
    def plot_interactive_pcs_ms_inverse(self, pcs_mean_softmax_scores):
        ...
        fig = make_subplots(
            ...
            cols=1,
            subplot_titles=(
                "Distribution of PCS Scores as Uncertainty",
        ...
        fig.add_trace(
            plotly_graph_objects.Histogram(
                x=pcs_inverse, name="PCS Scores", marker_color="skyblue"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            plotly_graph_objects.Scatter(
                ...
                mode="lines",
                name=f"95th percentile: {uncertainty_threshold_pcs:.2f}",
        ...
        fig.add_trace(
            plotly_graph_objects.Scatter(
                ...
                y=[0, max(np.histogram(pcs_scores, bins="auto")[0])],
                mode="lines",
                name=f"Mean PCS: {np.mean(pcs_scores):.2f}",
        ...
        fig.add_trace(
            plotly_graph_objects.Histogram(
                x=mean_softmax_inverse,
                name="Mean Softmax Scores",
                marker_color="lightgreen",
            ),
            row=2,
            col=1,
        ...
        fig.add_trace(
            plotly_graph_objects.Scatter(
                ...
                mode="lines",
                name=f"95th percentile: {uncertainty_threshold_mean_softmax:.2f}",
                line=dict(color="red", dash="dash"),
            ),
            row=2,
            col=1,
        ...
        fig.add_trace(
            plotly_graph_objects.Scatter(
                ...
                y=[0, max(np.histogram(mean_softmax_scores, bins="auto")[0])],
                mode="lines",
                name=f"Mean Softmax: {np.mean(mean_softmax_scores):.2f}",
                line=dict(color="yellow", dash="dash"),
            ),
            row=2,
            col=1,
    ...
    def plot_dist_entropy_scores(self, entropy_scores: list) -> None:
        ...
        plt.figure(figsize=(20, 10))
        sns.histplot(
            entropy_scores, bins=50, kde=True, color="red", label="Entropy Scores"
        )
        plt.axvline(
    ...
    def plot_interactive_dist_entropy_scores(self, entropy_scores):
        ...
        fig.add_trace(
            plotly_graph_objects.Histogram(
                x=entropy_scores, name="Predictive Entropy", marker_color="blue"
            ),
        )

        fig.update_layout(
            title_text="Histogram of Predictive Entropy",
    ...
    def high_uncertain_inputs(
        self, entropy_scores: list, x_test: np.ndarray, num_samples: int = 25
    ) -> None:
    ...
    def plot_predictive_conf_entropy_scores(
        self, predictive_confidence: list, entropy_scores: list
    ) -> None:
    ...
    def plot_interactive_predictive_conf_entropy_scores(
